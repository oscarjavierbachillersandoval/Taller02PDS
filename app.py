...
def load_data():
    ...
    datosenergia_df = pd.read_csv('datos_energia.csv')
    datosenergia_df['time'] = pd.to_datetime(datosenergia_df['time'])
    datosenergia_df.set_index('time', inplace=True)
    return datosenergia_df
